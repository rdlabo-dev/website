...
Present `PhotoEditorPage` in an Ionic modal. Call this after [Installation](/docs/readme#installation).
...
**Required.** Use `dark` for a dark/black `ion-toolbar` and `light` for a light/white toolbar. The library cannot infer toolbar appearance from CSS, translucency, or runtime theme overrides. See [Theme](/docs/theme).
