...
画面に反映される前に、リストのグループ漏れを検出します。Ionic Angularアプリでは、`@rdlabo/eslint-plugin-rules` がiOS 27で使うマークアップを検査します。
...
レイアウト例は [ion-item-groupの使用方法](/ionic-theme-ios27/docs/using-ion-item-group)、対応構造と自動修正は [ルールリファレンス](/eslint-plugin-rules/docs/rules/require-ion-item-group) を参照してください。
